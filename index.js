...
const server = http.createServer((req, res) => {
  ...
  const [tmp, app, screenName, id, address] = pathname.split('/')
  ...
  if (app == 't' && id && /^[a-z0-9_]+$/i.test(screenName) && screenName.length <= 15 && /^\d{18,20}$/.test(id) && id.length < 20 && web3.utils.isAddress(address)) {
  ...
  } else if (app == 'w' && id && /^\d+$/i.test(screenName) && screenName.length <= 15 && /^[a-z0-9_]+$/.test(id) && id.length < 12 && web3.utils.isAddress(address)) {
    ...
    .then(function (tweet) {
      if (tweet.text) {
        ...
        if (/^0x[0-9a-f]{130}/.test(content) && /^\w+$/.test(userId)) {
          ...
          const msgHash = utils.hashPersonalMessage(utils.toBuffer(`weibo/${userId}@tweedentity`))
          const sgn = utils.stripHexPrefix(content)
          const r = new Buffer(sgn.slice(0, 64), 'hex')
          const s = new Buffer(sgn.slice(64, 128), 'hex')
          const v = parseInt(sgn.slice(128, 130), 16) // + 27
